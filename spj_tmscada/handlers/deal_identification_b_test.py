...
class TestDealIdentificationB(unittest.TestCase):
    def setUp(self):
    ...
    @patch('handlers.deal_identification_b.rsa_sign_data')
    def test_pid_identification_b(self, mock_rsa_sign):
        # Mock successful signing
        mock_rsa_sign.return_value = b'SignatureBytes'
        
        hex_data = self._read_hex_from_file('deal_identification_b.txt')
        data = binascii.unhexlify(hex_data)
        
        # Standardized call: (server, addr, data)
        deal_identification_b(self.server, self.client_addr, data)
        
        # Verify sendto called
        self.server.sock.sendto.assert_called()
        
        # Verify arg structure
        args, _ = self.server.sock.sendto.call_args
        resp = args[0]
        self.assertGreater(len(resp), 22)
        # Verify Version=1 at offset 8
        import struct
